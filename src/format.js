...
window.exponentialFormat = function exponentialFormat(num, precision) {
	...
	if (!endSet.has('e')) end = addCommas(end.replace(/-/g, ''));
	if (e.lt(0)) end = `-${end}`;
	return `${start}e${end}`;
};
...
window.format = function format(decimal, precision = 2) {
	...
	return decimal.mag >= 1000 ? commaFormat(decimal, 0) : regularFormat(decimal, precision);
};
...
window.formatTime = function formatTime(s) {
	...
	if (s < 31556952) return `${formatWhole(Math.floor(s / 86400) % 365)}d ${formatWhole(Math.floor(s / 3600) % 24)}h ${formatWhole(Math.floor(s / 60) % 60)}m ${format(s % 60)}s`;
...
window.slog10 = function slog10(num) {
	let slogmag = num.mag < 10 ? Math.log10(num.mag) : num.mag < 1e10 ? 1 + Math.log10(Math.log10(num.mag)) : 2 + Math.log10(Math.log10(Math.log10(num.mag)));
...
window.norm = function norm(num) {
	...
	// Handle edge cases for zero or infinite values
	if (num.sign === 0 || (num.mag === 0 && num.layer === 0) ||
		(num.mag === Number.NEGATIVE_INFINITY && num.layer > 0 && Number.isFinite(num.layer))) {
		num.sign = 0;
		num.mag = 0;
		num.layer = 0;
		return num;
	...
	// Extract sign from negative magnitude at layer 0
	...
	if (num.mag === Number.POSITIVE_INFINITY || num.layer === Number.POSITIVE_INFINITY ||
		num.mag === Number.NEGATIVE_INFINITY || num.layer === Number.NEGATIVE_INFINITY) {
	...
	if (absmag >= EXP_LIMIT) {
		...
		// Shift down layers if necessary
		...
		// Handle sign extraction at layer 0
		if (num.layer === 0) {
			if (num.mag < 0) {
				num.mag = -num.mag;
				num.sign = -num.sign;
...
window.scale = function scale(s) {
    ...
    // Handle zero, infinite, NaN, and non-finite values
    if (s.eq(0)) {
    ...
    // Handle negative values (convert to positive) and very small values (reciprocal)
    if (s.lt(0)) {
        isNegative = true;
        s = s.abs();
    }
    ...
    const scaleValues = [282e-44, 6.7116e-40, 723e-29, 21952e-30, 5e-18, 9e-14, 62e-9, 5e-5, .003555, 1, 52.995764976, 1e3, 67596.84, 25e5, 26006e5, 562e9, 12232e12, 66988e16, 108321e19, 14313e23, 1.412e30, 5e35, 8e39, 17e47, 17e50, 3e58, 33e63, 5e71, 35e74, 34e82];
    ...
    const scaleHeightValues = [5.6692908e-8, .000125787444, .11811024, 66, 399.21258, 21600, 32598.43, 2090281.8, 98622e3, 3009921e3, 9094488e4, 3533808e7, 10626162e8, 9430902e12, 195348e18, 11621016e18, 44919702e19, 2094018e23];

    // Handle scaling for smaller numbers
    if (s.lt("e1000")) {
    ...
    // Handle scaling for larger numbers (in terms of time for bacteria growth)
    if (s.lt("ee15")) {
        s = s.log2();
    ...
    // Handle extremely large numbers (power tower scaling)
