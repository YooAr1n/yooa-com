    ...
    methods: {
        updateNewsArray() {
            this.newsArray = [
                ...
                // Eishel
                [() => "HEI! A new presence has entered the system. Awkward silence cannot survive.", true, "eishel_1"],
                [() => "Eishel said 'HEI!' and three shy people accidentally made eye contact. Progress.", true, "eishel_2"],
                [() => "Breaking: Eishel asked the question everyone was thinking. The professor paused. Then admitted she was right.", Math.random() < 0.5, "eishel_3"],
                [() => "System Audit in progress… Eishel detected logical inconsistency in the lecture. Raising hand now.", Math.random() < 0.45, "eishel_4"],
                [() => "Eishel walked in. The room's SUS level dropped by 40%. Awkward silence filed for retirement.", Math.random() < 0.4, "eishel_5"],
                [() => "HEI! Protocol activated. Social lag bypassed. Connection established in 0.00917 seconds.", Math.random() < 0.45, "eishel_6"],
                [() => "Eishel's volume calibration: Normal people = 60dB. Eishel at rest = 'HEI!' = 9,170dB. Satellites confirmed.", Math.random() < 0.4, "eishel_7"],
                [() => "Eishel asked 'but WHY does it work?' — the textbook had no answer. She filed a complaint.", Math.random() < 0.35, "eishel_8"],
                [() => "Breaking: Eishel initiated conversation in a room of 12 people who were all too shy to start. Casualties: zero. Bonds: formed.", Math.random() < 0.35, "eishel_9"],
                [() => "Eishel's 'HEI!' was so loud it accidentally enrolled three bystanders in the class.", Math.random() < 0.3, "eishel_10"],
                [() => "System Auditor Eishel detected a gap in understanding. She asked. The class collectively exhaled in relief.", Math.random() < 0.35, "eishel_11"],
                [() => "Eishel bypassed Brain-Lag in 0.1 seconds. Method: say 'HEI!' before the brain catches up.", Math.random() < 0.3, "eishel_12"],
                [() => "Eishel's social energy is so high it registers on weather instruments. Meteorologists are confused.", Math.random() < 0.3, "eishel_13"],
                [() => "Breaking: Eishel graded the lecture. It got a B+. She had notes.", Math.random() < 0.25, "eishel_14"],
                [() => "Eishel smiled loudly. Nobody knows how. It worked.", Math.random() < 0.25, "eishel_15"],
                [() => "Eishel's 'HEI!' counts as a natural disaster in 7 countries and a welcome greeting in all of them.", Math.random() < 0.2, "eishel_16"],
                [() => "Scientists ran diagnostics on Eishel's social output. Result: unmeasurable. Unit of measure: HEI.", Math.random() < 0.2, "eishel_17"],
                [() => "Eishel entered the classroom and the group chat came to life 9.17 seconds later. Causation confirmed.", Math.random() < 0.25, "eishel_18"],
                [() => "Eishel's Clouds-to-Atmosphere arc triggered mid-discussion. Logic mode engaged. The professor took notes.", Math.random() < 0.2, "eishel_19"],
                [() => "Breaking: Eishel just De-SUS'd a situation Ahn Jean spent 20 minutes creating. Balance restored.", Math.random() < 0.25, "eishel_20"],
                [() => "Eishel's presence doesn't just break the ice. It evaporates it and replaces it with warm vibes.", Math.random() < 0.3, "eishel_21"],
                [() => "Eishel once said 'HEI!' in an empty room. The walls made friends with each other.", Math.random() < 0.2, "eishel_22"],
                [() => "Warning: Eishel's Logic Mode is online. Incorrect explanations, prepare for audit.", Math.random() < 0.25, "eishel_23"],
                [() => "Eishel's score fluctuates: Clouds when 'HEI!' mode is active. Atmosphere when Logic kicks in. Earth? Rare event.", Math.random() < 0.2, "eishel_24"],
                [() => "Eishel asked 'wait, how does that actually work?' — the whole class leaned in. This is her power.", Math.random() < 0.3, "eishel_25"],

                // Eishel × Others
                [() => "Eishel said 'HEI!' Euryle responded with MIRACLE energy. The hallway is now canonically iconic.", Math.random() < 0.3, "eishel_euryle_1"],
                [() => "Eishel broke the ice. Euryle painted it pink and gold. The room never recovered.", Math.random() < 0.25, "eishel_euryle_2"],
                [() => "Eishel walked up and said 'HEI!' Ahn Jean immediately called an emergency meeting. Classic.", Math.random() < 0.3, "eishel_ahn_1"],
                [() => "Eishel tried to De-SUS Ahn Jean. Ahn Jean generated new SUS mid-audit. Draw.", Math.random() < 0.25, "eishel_ahn_2"],
                [() => "Eishel greeted Kimberly loudly. Kimberly replied with one blink. Connection: established.", Math.random() < 0.3, "eishel_kim_1"],
                [() => "Eishel audited Kimberly's dark logic. Surprisingly valid. She filed an approving report.", Math.random() < 0.2, "eishel_kim_2"],
                [() => "Eishel and Yssabelle crossed paths. PALDO energy and HEI! energy collided. Local economy fluctuated.", Math.random() < 0.3, "eishel_yssa_1"],
                [() => "Eishel asked why. Yssabelle answered in vibes. Eishel accepted it. Growth.", Math.random() < 0.25, "eishel_yssa_2"],
                [() => "Eishel's HEI! reached Alaine. Alaine responded with a calm smile. The universe balanced.", Math.random() < 0.25, "eishel_alaine_1"],
                [() => "Eishel audited the vibe. Alaine certified it golden. Both satisfied.", Math.random() < 0.2, "eishel_alaine_2"],
                [() => "Eishel asked Serille a question mid-sprint. Serille answered without slowing down. Efficiency unchanged.", Math.random() < 0.25, "eishel_serille_1"],
                [() => "Eishel's loud HEI! collided with Serille's serene aura. Result: calm productivity at high volume.", Math.random() < 0.2, "eishel_serille_2"],
                [() => "Eishel greeted Regina. Regina nodded with regal precision. HEI! Protocol: royally acknowledged.", Math.random() < 0.25, "eishel_regina_1"],
                [() => "Eishel raised a question. Regina restructured it into an actionable insight. Teamwork.", Math.random() < 0.2, "eishel_regina_2"],

                // Eishel Location / Rare
                [() => "BREAKING: Eishel said 'HEI!' in Hà Nội. Three nearby phở vendors waved back instinctively.", Math.random() < 0.35, "eishel_hanoi_1"],
                [() => "Report: Eishel audited Hà Nội traffic logic and found it 'surprisingly efficient, actually.'", Math.random() < 0.3, "eishel_hanoi_2"],
                [() => "BREAKING: Eishel said 'HEI!' in Seoul. A stranger replied. They are now study partners.", Math.random() < 0.35, "eishel_seoul_1"],
                [() => "RARE: Eishel entered full Logic Mode and Atmosphere Level simultaneously. The professor stopped the lecture to clap.", Math.random() < 0.04, "eishel_rare_1"],
                [() => "RARE: Eishel's HEI! was so precisely timed it synchronized with YooA's chorus. Scientists wept.", Math.random() < 0.02, "eishel_rare_2"],
                ...
                // YooA × Friends
                [() => "YooA just waved at the screen. Euryle screamed. The game gained 9.17% speed from sheer MIRACLE energy.", true, "yooa_friends_1"],
                [() => "YooA appeared in the game. Euryle's hands started shaking. She still got Space level. Respect.", Math.random() < 0.5, "yooa_friends_2"],
                [() => "YooA smiled mid-cutscene. Euryle paused the game to document it. 47 screenshots taken.", Math.random() < 0.45, "yooa_friends_3"],
                [() => "YooA's theme played. Euryle mouthed every lyric perfectly. Alaine hummed along. The game felt emotional.", Math.random() < 0.4, "yooa_friends_4"],
                [() => "YooA asked who her biggest fan is. Euryle and Alaine answered simultaneously. The game lagged from the tie.", Math.random() < 0.35, "yooa_friends_5"],
                [() => "YooA's dance animation triggered. Ahn Jean said 'SUS, she's too perfect.' She was ejected from the comment.", Math.random() < 0.3, "yooa_friends_6"],
                [() => "Kimberly watched YooA's cutscene in silence. The darkness around her glowed for exactly one second. Nobody mentioned it.", Math.random() < 0.3, "yooa_friends_7"],
                [() => "Yssabelle watched YooA's intro and whispered 'she's giving PALDO goddess energy.' We agree.", Math.random() < 0.35, "yooa_friends_8"],
                [() => "YooA appeared on screen. Eishel said 'HEI!' YooA's sprite turned to look. Scientists are investigating.", Math.random() < 0.3, "yooa_friends_9"],
                [() => "Serille completed every task before YooA's opening animation finished. YooA looked impressed.", Math.random() < 0.25, "yooa_friends_10"],
                [() => "Regina critiqued YooA's in-game UI design. YooA nodded. The UI updated. Nobody coded that.", Math.random() < 0.2, "yooa_friends_11"],
                [() => "YooA's prestige cutscene played. Euryle watched it 9 times. Alaine watched it 17 times. The session timer gave up.", Math.random() < 0.3, "yooa_friends_12"],
                [() => "YooA Incremental asked 'Who plays this game?' Euryle raised her hand so hard she knocked over her water bottle.", Math.random() < 0.35, "yooa_friends_13"],
                [() => "YooA's sprite blinked. Euryle said 'she looked at me.' No one corrected her. She was probably right.", Math.random() < 0.4, "yooa_friends_14"],

                // Arin × Friends
                [() => "Arin tried to autobuy upgrades for everyone. Ahn Jean's upgrades came back wrong. Very SUS upgrades.", Math.random() < 0.4, "arin_friends_1"],
                [() => "Arin autobuyed Yssabelle's upgrades and accidentally converted them all into PALDO. Economists notified.", Math.random() < 0.35, "arin_friends_2"],
                [() => "Arin looked at Kimberly's build. She nodded once and said 'dark but optimal.' High praise.", Math.random() < 0.3, "arin_friends_3"],
                [() => "Arin tried to autobuy for Euryle. Euryle already bought everything. Arin sat down quietly.", Math.random() < 0.35, "arin_friends_4"],
                [() => "Arin offered Eishel a tutorial. Eishel said 'HEI! I already audited the whole system.' Arin blinked.", Math.random() < 0.3, "arin_friends_5"],
                [() => "Arin checked Serille's progress and gasped. Serille had completed everything before Arin could help. A humbling moment.", Math.random() < 0.25, "arin_friends_6"],
                [() => "Arin tried to explain the upgrade tree to Ahn Jean. Ahn Jean nodded for 4 minutes and then asked 'so is this sus?'", Math.random() < 0.3, "arin_friends_7"],
                [() => "Arin handed Regina a list of optimized upgrades. Regina reorganized the list first. Then bought them. Arin respects this.", Math.random() < 0.25, "arin_friends_8"],
                [() => "Arin tried to guide Alaine through a prestige. Alaine was already done. Arin just smiled. 'Heartlight energy,' she said.", Math.random() < 0.3, "arin_friends_9"],
                [() => "Arin's autobuy triggered for Yssabelle during a math problem. Yssabelle got 110%. Arin was not credited. She's okay with that.", Math.random() < 0.3, "arin_friends_10"],

                // OH MY GIRL × Friends
                [() => "OH MY GIRL's theme started playing. Euryle stopped everything. Alaine closed her eyes. The game waited.", Math.random() < 0.45, "omg_friends_1"],
                [() => "Euryle ranked every OH MY GIRL song during a YooAmatter reset. She was not asked to. She finished anyway.", Math.random() < 0.4, "omg_friends_2"],
                [() => "Alaine's Heartlight resonated with MIRACLE frequency the moment 'Closer' played. Scientists took notes.", Math.random() < 0.35, "omg_friends_3"],
                [() => "Hyojung coordinated the friend group's task schedule. They all completed at the same time. Suspiciously perfect.", Math.random() < 0.3, "omg_friends_4"],
                [() => "Mimi calculated Yssabelle's PALDO growth rate and concluded it matches OH MY GIRL's comeback interval. Eerie.", Math.random() < 0.25, "omg_friends_5"],
                [() => "Seunghee's voice calmed Ahn Jean's SUS for 3.7 seconds — a new world record.", Math.random() < 0.3, "omg_friends_6"],
                [() => "Yubin analyzed Kimberly's dark efficiency. Verdict: 'Somehow flawless.' Yubin doesn't give out 'flawless' easily.", Math.random() < 0.25, "omg_friends_7"],
                [() => "MIRACLE fans gathered: Euryle representing Space tier, Alaine representing Atmosphere. Combined energy: immeasurable.", Math.random() < 0.35, "omg_friends_8"],
                [() => "Eishel learned what MIRACLE fandom meant and said 'HEI! That makes sense for Euryle and Alaine actually.'", Math.random() < 0.3, "omg_friends_9"],
                [() => "Serille completed every OH MY GIRL discography task before the playlist shuffled. Mimi clapped. Once.", Math.random() < 0.2, "omg_friends_10"],
                [() => "Regina redesigned the MIRACLE fandom logo. OH MY GIRL saw it. They said nothing but the logo quietly updated.", Math.random() < 0.2, "omg_friends_11"],
                [() => "Ahn Jean tried to explain why YooA is SUS. The logic collapsed. Euryle filed a formal complaint.", Math.random() < 0.3, "omg_friends_12"],
                [() => "Kimberly once said 'YooA's dark concept era hits different.' Euryle framed this quote. It hangs in her room.", Math.random() < 0.25, "omg_friends_13"],
                [() => "Yssabelle noticed that every time OH MY GIRL plays, her PALDO sense activates. Correlation confirmed.", Math.random() < 0.3, "omg_friends_14"],

                // YooA Incremental × Friends (Gameplay)
                [() => "Euryle opened YooA Incremental. Immediately reached Space level. Did not explain how. Did not need to.", true, "yooa_inc_friends_1"],
                [() => "Ahn Jean played YooA Incremental and accidentally triggered a prestige 4 minutes in. SUS strat. It worked.", Math.random() < 0.4, "yooa_inc_friends_2"],
                [() => "Kimberly played in complete darkness. Somehow achieved Atmosphere level. No lights required apparently.", Math.random() < 0.35, "yooa_inc_friends_3"],
                [() => "Yssabelle found a math exploit in YooA Incremental. She called it 'PALDO routing.' Devs are reviewing.", Math.random() < 0.35, "yooa_inc_friends_4"],
                [() => "Alaine played YooA Incremental so calmly that the game's music tempo slowed down to match her energy.", Math.random() < 0.3, "yooa_inc_friends_5"],
                [() => "Serille completed the entire early game during a loading screen. The loading screen finished second.", Math.random() < 0.3, "yooa_inc_friends_6"],
                [() => "Regina opened YooA Incremental and immediately criticized the UI layout. Three things improved the next day.", Math.random() < 0.25, "yooa_inc_friends_7"],
                [() => "Eishel played YooA Incremental and audited every upgrade description. Filed 9 logic inconsistency reports. All valid.", Math.random() < 0.3, "yooa_inc_friends_8"],
                [() => "Euryle and Alaine both hit YooAmatter unlock at the same time. The server noted this as a 'MIRACLE sync event.'", Math.random() < 0.25, "yooa_inc_friends_9"],
                [() => "Ahn Jean spam-clicked the prestige button. Kimberly watched silently. Yssabelle took notes. Nobody stopped her.", Math.random() < 0.3, "yooa_inc_friends_10"],
                [() => "Eishel asked 'but why does YooAity work like this?' — the question nobody dared ask. The devs updated the tooltip.", Math.random() < 0.3, "yooa_inc_friends_11"],
                [() => "Serille's run is now used as the benchmark for optimal YooA Incremental efficiency. She doesn't know this.", Math.random() < 0.2, "yooa_inc_friends_12"],
                [() => "Alaine reached YooAmatter before everyone else and waited patiently. She had snacks. The Heartlight glowed.", Math.random() < 0.25, "yooa_inc_friends_13"],
                [() => "Yssabelle accidentally unlocked a secret PALDO route in YooA Incremental. Economists are still recovering.", Math.random() < 0.25, "yooa_inc_friends_14"],
                [() => "Kimberly's YooA Incremental save file is titled 'void.yooa'. Nobody asked. Everyone respects it.", Math.random() < 0.2, "yooa_inc_friends_15"],
                [() => "Regina's save file has perfect formatting. Every resource is a round number. The game did not do this.", Math.random() < 0.2, "yooa_inc_friends_16"],
                [() => "Eishel hit Clouds level and said 'I was auditing the upgrades, I wasn't even trying.' This is canon.", Math.random() < 0.25, "yooa_inc_friends_17"],
                [() => "The group played YooA Incremental together. Euryle topped the scores. Alaine was second. Eishel asked why. Still valid.", Math.random() < 0.3, "yooa_inc_friends_18"],
                [() => "Ahn Jean's YooA Incremental strategy is classified. Not by choice — the file just says 'trust me' and crashes.", Math.random() < 0.25, "yooa_inc_friends_19"],
                [() => "The whole friend group is now in YooA Incremental lore. They don't know yet. The news ticker is telling them now.", true, "yooa_inc_friends_20"],

                // Eishel × Math
                [() => "Eishel looked at the math problem and said 'HEI! Wait. Actually. Give me a second.' She got it right.", true, "eishel_math_1"],
                [() => "Eishel's math strategy: say 'HEI!' loudly, then let the confusion buy time to actually think. 80% success rate.", Math.random() < 0.5, "eishel_math_2"],
                [() => "Eishel asked 'but WHY is the answer that?' for 4 consecutive problems. The professor gave up and explained everything. Eishel won.", Math.random() < 0.45, "eishel_math_3"],
                [() => "Eishel's Logic Mode activated mid-exam. She solved 3 problems in silence. The class was concerned. The grades were not.", Math.random() < 0.4, "eishel_math_4"],
                [() => "Eishel said 'HEI! Is that even a valid formula?' It wasn't. The professor thanked her. The class owed her.", Math.random() < 0.45, "eishel_math_5"],
                [() => "Eishel's score went from Earth to Atmosphere the moment she stopped saying 'HEI!' and started saying 'wait, actually...'", Math.random() < 0.4, "eishel_math_6"],
                [() => "Eishel audited the entire exam rubric before answering a single question. She finished first anyway.", Math.random() < 0.35, "eishel_math_7"],
                [() => "Eishel does not guess on math problems. She audits them until they confess the answer.", Math.random() < 0.35, "eishel_math_8"],
                [() => "Eishel's exam paper has more questions written on it than answers. The professor still gave her partial credit for the questions.", Math.random() < 0.3, "eishel_math_9"],
                [() => "Eishel calculated the probability of her getting a high score. The answer was 'depends on HEI! frequency.' Accurate.", Math.random() < 0.3, "eishel_math_10"],
                [() => "Eishel asked if the formula was derived or memorized. The professor said derived. Eishel then re-derived it live. It matched.", Math.random() < 0.25, "eishel_math_11"],
                [() => "Eishel's math notebook alternates between 'HEI!' and correct solutions. No in-between. No drafts. Just vibes and answers.", Math.random() < 0.3, "eishel_math_12"],
                [() => "Eishel caught a computational error on the board, said 'HEI! that's wrong,' and fixed it before the professor erased it.", Math.random() < 0.3, "eishel_math_13"],
                [() => "Eishel's System Audit detected an incorrect formula in the textbook. Edition 7. Page 214. She emailed the publisher.", Math.random() < 0.2, "eishel_math_14"],
                [() => "Eishel solved the hardest problem on the exam and then wrote 'HEI!' at the top as a signature. It was graded. She passed.", Math.random() < 0.2, "eishel_math_15"],

                // HEI (Higher Education Institution / HEI! Protocol)
                [() => "HEI officially stands for Higher Education Institution. For Eishel, it stands for 'I have arrived and silence is over.'", true, "hei_1"],
                [() => "Professors now include 'HEI! Protocol active' in their attendance sheets. One student. You know who.", Math.random() < 0.5, "hei_2"],
                [() => "The acronym HEI was invented for academic purposes. Eishel repurposed it for social ones. Both uses are correct.", Math.random() < 0.45, "hei_3"],
                [() => "Every Higher Education Institution in the country now has an unofficial mascot: the person who says 'HEI!' too loud.", Math.random() < 0.4, "hei_4"],
                [() => "HEI frequency detected: one per hallway, three per classroom, infinite per Eishel. Standards confirmed.", Math.random() < 0.4, "hei_5"],
                [() => "The Technological Institute of the Philippines is a Higher Education Institution. Eishel calls it home. Both definitions apply.", Math.random() < 0.45, "hei_6"],
                [() => "TIP is an HEI. Eishel attends TIP. Eishel says 'HEI!' constantly. The loop is complete. The lore is sealed.", Math.random() < 0.4, "hei_7"],
                [() => "Eishel discovered that her school is literally called an HEI. She has not stopped laughing since February 2026.", Math.random() < 0.4, "hei_8"],
                [() => "Report: Eishel greeted the campus bulletin board with 'HEI!' It is a Higher Education Institution notice board. Technically correct.", Math.random() < 0.35, "hei_9"],
                [() => "Linguistics professors confirm: 'HEI!' is now a valid dual-use term — academic institution AND Eishel's battle cry.", Math.random() < 0.35, "hei_10"],
                [() => "Breaking: Eishel walked into class and shouted 'HEI!' The professor said 'yes, this is an HEI.' Nobody recovered.", Math.random() < 0.35, "hei_11"],
                [() => "The Commission on Higher Education has been made aware of the HEI! Protocol. They are reviewing its academic merit.", Math.random() < 0.3, "hei_12"],
                [() => "Eishel says 'HEI!' the same way other people say 'good morning.' One of these is more effective.", Math.random() < 0.35, "hei_13"],
                [() => "Research confirms: environments where 'HEI!' is said more frequently show higher group participation rates. Eishel is a phenomenon.", Math.random() < 0.3, "hei_14"],
                [() => "Eishel once said 'HEI!' to a professor who responded 'yes, welcome to this Higher Education Institution.' She loved it.", Math.random() < 0.3, "hei_15"],
                [() => "The word 'HEI!' now has two definitions. Definition 1: Higher Education Institution. Definition 2: Eishel has entered the room.", Math.random() < 0.3, "hei_16"],
                [() => "HEI Protocol initiation sequence: Eishel enters room → says 'HEI!' → awkward silence dissolves → productivity increases → class proceeds. Flawless.", Math.random() < 0.25, "hei_17"],
                [() => "Scientists asked: what gives 'HEI!' its power? Answer: it is simultaneously a greeting, an institution, and a warning.", Math.random() < 0.25, "hei_18"],
                [() => "Eishel's 'HEI!' is registered as both a social greeting and an accreditation type. She did not plan this. It happened.", Math.random() < 0.2, "hei_19"],
                [() => "BREAKING: A new rule was added to the student handbook: 'HEI! is encouraged.' The rule author is unknown. Suspects: one.", Math.random() < 0.2, "hei_20"],
                [() => "The universe has two constants: the speed of light, and the fact that Eishel will say 'HEI!' within 9.17 seconds of arriving.", true, "hei_21"],

                // HEI × Math
                [() => "Eishel entered the math room, said 'HEI!', and the equation on the board rearranged itself into the correct answer. Peer review pending.", Math.random() < 0.35, "hei_math_1"],
                [() => "The formula for HEI! energy output: (volume × confidence) ÷ awkward silence = breakthrough understanding. Eishel derived this herself.", Math.random() < 0.3, "hei_math_2"],
                [() => "Eishel proved that saying 'HEI!' before a math problem increases solve rate by 9.17%. Sample size: every exam she's taken.", Math.random() < 0.3, "hei_math_3"],
                [() => "Mathematicians have added a new variable to social dynamics equations: h, representing the number of 'HEI!'s per session.", Math.random() < 0.25, "hei_math_4"],
                [() => "The integral of Eishel's 'HEI!' frequency over one school day equals the exact amount of class participation the professor needed.", Math.random() < 0.25, "hei_math_5"],
                [() => "Eishel graphed her 'HEI!' usage vs her exam scores. Positive correlation. Strong R² value. She submitted it as extra credit.", Math.random() < 0.25, "hei_math_6"],
                [() => "New math theorem: if HEI! → understanding, and understanding → grade, then HEI! → grade. Eishel's proof is one page long.", Math.random() < 0.2, "hei_math_7"],
                [() => "Eishel said 'HEI! is this a Higher Education Institution problem or a me problem?' The answer was both. She solved it.", Math.random() < 0.25, "hei_math_8"],

                // Eishel Funny Math Extended
                [() => "Eishel's study method: read once, say 'HEI!' to process, re-read with Logic Mode active, submit. Clouds to Atmosphere pipeline.", Math.random() < 0.35, "eishel_fun_math_1"],
                [() => "Eishel answered a math question so confidently the professor assumed she had studied. She had just said 'HEI!' and guessed.", Math.random() < 0.3, "eishel_fun_math_2"],
                [() => "Eishel once yelled 'HEI!' at a derivative. It resolved itself. Nobody can explain this. Calculus is scared of her.", Math.random() < 0.3, "eishel_fun_math_3"],
                [() => "Eishel's test scores follow the same pattern as her 'HEI!' count: unpredictable, occasionally brilliant, always memorable.", Math.random() < 0.3, "eishel_fun_math_4"],
                [() => "Eishel holds the record for most clarifying questions asked in a single math lecture: 9. All were valid. All changed the class.", Math.random() < 0.25, "eishel_fun_math_5"],
                [() => "Eishel calculated her own GPA trajectory and said 'HEI! this is actually fixable.' She then fixed it. System Audit success.", Math.random() < 0.25, "eishel_fun_math_6"],
                [() => "Eishel's math is: 40% logic, 40% auditing the question, 17% saying 'HEI!', and 3% pure confidence. Results: mostly correct.", Math.random() < 0.3, "eishel_fun_math_7"],
                [() => "Eishel discovered that if you audit a math problem hard enough, it eventually just tells you the answer. She calls this 'HEI! method'.", Math.random() < 0.25, "eishel_fun_math_8"],

                // Fan Hearts
                [() => "Fan Hearts collected! MIRACLEs are pouring love into the system.", player.YooAity.FanHearts?.gte(1), "fanhearts_1"],
                [() => "Fan Hearts are accumulating faster than Euryle can screenshot them.", player.YooAity.FanHearts?.gte(100), "fanhearts_2"],
                [() => "Breaking: Fan Hearts now power a measurable portion of the YooAverse.", player.YooAity.FanHearts?.gte(1e6), "fanhearts_3"],
                [() => "Fan Hearts detected at critical mass. MIRACLEs are not slowing down.", player.YooAity.FanHearts?.gte(1e12), "fanhearts_4"],
                [() => "Scientists confirm: Fan Hearts emit a frequency only Euryle and Alaine can hear.", Math.random() < 0.4, "fanhearts_5"],
                [() => "Fan Hearts overflowing. Arin is trying to autobuy containers for them.", Math.random() < 0.35, "fanhearts_6"],
                [() => "Warning: Fan Heart density approaching structural limits of the cosmos.", player.YooAity.FanHearts?.gte(1e30), "fanhearts_7"],
                [() => "Each Fan Heart is a MIRACLE sending love to YooA. Each one counts. Even Ahn Jean's.", Math.random() < 0.3, "fanhearts_8"],
                [() => "Fan Hearts now outweigh the total mass of known dark matter. MIRACLEs did this.", player.YooAity.FanHearts?.gte(1e50), "fanhearts_9"],
                [() => "Report: Euryle's Fan Heart count is suspiciously high. She has no explanation. She needs none.", Math.random() < 0.35, "fanhearts_10"],
                [() => "Alaine's Heartlight and Fan Hearts synchronized perfectly. The resonance caused a solar flare.", Math.random() < 0.3, "fanhearts_11"],
                [() => "Eishel audited the Fan Heart formula and said 'HEI! This math doesn't add up.' It does. It just exceeds mortal comprehension.", Math.random() < 0.3, "fanhearts_12"],
                [() => "Kimberly counted her Fan Hearts in silence. The number was unsettling. She approved.", Math.random() < 0.25, "fanhearts_13"],
                [() => "Yssabelle converted Fan Hearts into PALDO somehow. Economists are not okay.", Math.random() < 0.25, "fanhearts_14"],
                [() => "Ahn Jean asked 'are Fan Hearts sus?' The answer is no. She collected them anyway.", Math.random() < 0.3, "fanhearts_15"],
                [() => "Fan Hearts per song is now measurable in astronomical units. Songs are getting more powerful.", Math.random() < 0.25, "fanhearts_16"],
                [() => "Breaking: Fan Heart gain just doubled. The MIRACLEs are learning.", player.YooAity.FanHearts?.gte(1e100), "fanhearts_17"],
                [() => "Fan Hearts are now the most abundant resource in the YooAverse. More than stars. More than YooAmatter. Almost.", player.YooAity.FanHearts?.gte("e200"), "fanhearts_18"],

                // MIRACLEs (Fandom tab context)
                [() => "MIRACLE count rising. Every new fan brings YooA one step closer to omnipresence.", gameLayers.OMG.getMIRACLEs().gte(1), "fandom_miracle_1"],
                [() => "The fandom grows. MIRACLEs are multiplying faster than the upgrade tree can handle.", gameLayers.OMG.getMIRACLEs().gte(100), "fandom_miracle_2"],
                [() => "MIRACLEs united: their collective love boosts all member lights simultaneously.", gameLayers.OMG.getMIRACLEs().gte(1000), "fandom_miracle_3"],
                [() => "Breaking: MIRACLE count just crossed a new threshold. Fan Hearts are responding.", gameLayers.OMG.getMIRACLEs().gte(1e4), "fandom_miracle_4"],
                [() => "The fandom is self-sustaining now. MIRACLEs generate more MIRACLEs. Physics weeps.", gameLayers.OMG.getMIRACLEs().gte(1e6), "fandom_miracle_5"],
                [() => "Celestial Market Saturation warning: too many MIRACLEs. The universe is at capacity. They kept going.", gameLayers.OMG.getMIRACLEs().gte(2e5), "fandom_miracle_6"],
                [() => "MIRACLE gain divided by Celestial Market Saturation. The K-pop universe is full. Expand it anyway.", gameLayers.OMG.getMIRACLEs().gte(3e5), "fandom_miracle_7"],
                [() => "Euryle personally welcomed every MIRACLE. She is still going. She will not stop.", Math.random() < 0.35, "fandom_miracle_8"],
                [() => "Alaine's golden pulse attracted 9.17 new MIRACLEs per second. Heartlight energy confirmed.", Math.random() < 0.3, "fandom_miracle_9"],
                [() => "Eishel asked 'HEI! What even is a MIRACLE?' Euryle answered for 47 minutes. Eishel is now a MIRACLE.", Math.random() < 0.3, "fandom_miracle_10"],
                [() => "Kimberly became a MIRACLE in secret. Nobody knew until the Fan Heart count jumped.", Math.random() < 0.25, "fandom_miracle_11"],
                [() => "Yssabelle calculated that at current MIRACLE growth, the fandom will outnumber atoms by next semester.", Math.random() < 0.25, "fandom_miracle_12"],
                [() => "Ahn Jean joined the fandom. She said it was 'not suspicious at all.' It was a little suspicious.", Math.random() < 0.3, "fandom_miracle_13"],

                // Songs
                [() => "A new song plays. Fan Hearts begin generating. MIRACLEs are listening.", true, "song_1"],
                [() => "Song detected in the YooAverse. Fan Heart gain activated. Royalties pending.", true, "song_2"],
                [() => "Breaking: YooA released a new song. Every MIRACLE heard it simultaneously. Satellites confirmed.", Math.random() < 0.45, "song_3"],
                [() => "The song is playing. Euryle mouthed every word. Alaine hummed the melody. The game felt something.", Math.random() < 0.4, "song_4"],
                [() => "Each song generates Fan Hearts proportional to MIRACLE count. The math is beautiful. Eishel verified it.", Math.random() < 0.35, "song_5"],
                [() => "Song activated. Kimberly listened in darkness. One Fan Heart was generated per blink.", Math.random() < 0.3, "song_6"],
                [() => "Yssabelle noticed that every song plays converts to PALDO at a 9.17:1 ratio. She did not explain how.", Math.random() < 0.3, "song_7"],
                [() => "Ahn Jean tried to vent during the song. The melody stopped her. Even SUS yields to YooA.", Math.random() < 0.3, "song_8"],
                [() => "Serille completed all her tasks before the intro finished. She stayed for the bridge.", Math.random() < 0.25, "song_9"],
                [() => "Regina critiqued the song's visual presentation. She approved of the aesthetic. High honor.", Math.random() < 0.25, "song_10"],
                [() => "Eishel said 'HEI! This song slaps' mid-System Audit. The audit was paused. The song was not.", Math.random() < 0.3, "song_11"],
                [() => "The song ended. Fan Hearts peaked. Euryle replayed it. Fan Hearts peaked again.", Math.random() < 0.35, "song_12"],
                [() => "Scientists studied what happens when MIRACLEs listen to a song simultaneously. The result was Fan Hearts. Lots of them.", Math.random() < 0.3, "song_13"],
                [() => "Alaine listened to the song with her eyes closed. Her Heartlight pulsed in perfect time with the beat.", Math.random() < 0.25, "song_14"],

                // Royalties / Money
                [() => "Royalties unlocked! Songs now generate income alongside Fan Hearts. The fandom economy is real.", hasUpgrade("Fandom", 13), "royalties_1"],
                [() => "Money flowing in from songs. The MIRACLEs are funding the YooAverse.", hasUpgrade("Fandom", 13), "royalties_2"],
                [() => "Breaking: Royalty income just exceeded the GDP of a small planet.", hasUpgrade("Fandom", 13) && Math.random() < 0.4, "royalties_3"],
                [() => "Yssabelle saw the royalty numbers and whispered 'PALDO.' She was not wrong.", hasUpgrade("Fandom", 13) && Math.random() < 0.4, "royalties_4"],
                [() => "Money gain per song scales with MIRACLEs. The bigger the fandom, the richer the YooAverse.", hasUpgrade("Fandom", 13) && Math.random() < 0.35, "royalties_5"],
                [() => "Eishel audited the royalty system and said 'HEI! This is actually a valid revenue model.' She filed a report.", hasUpgrade("Fandom", 13) && Math.random() < 0.3, "royalties_6"],
                [() => "Regina reviewed the royalty income statement. She nodded. Once. That is the highest possible approval.", hasUpgrade("Fandom", 13) && Math.random() < 0.25, "royalties_7"],
                [() => "Kimberly calculated royalty projections in the dark. The spreadsheet glowed ominously. Numbers were correct.", hasUpgrade("Fandom", 13) && Math.random() < 0.25, "royalties_8"],
                [() => "Arin tried to autobuy with the royalty money. Euryle stopped her. Some things must be done manually.", hasUpgrade("Fandom", 13) && Math.random() < 0.3, "royalties_9"],
                [() => "Royalty income report: total earnings are now inexpressible in standard notation. This is a good sign.", hasUpgrade("Fandom", 13) && player.YooAity?.stream?.money?.gte("e100"), "royalties_10"],

                // Celestial Market Saturation
                [() => "Celestial Market Saturation reached. The K-pop universe is full. MIRACLEs are still buying tickets.", gameLayers.OMG.getMIRACLEs().gte(2e5), "cms_1"],
                [() => "Warning: Celestial Market Saturation active. MIRACLE gain is being divided. They divided anyway.", gameLayers.OMG.getMIRACLEs().gte(2e5), "cms_2"],
                [() => "The K-pop universe has reached its MIRACLE capacity. New universes are being opened to handle overflow.", gameLayers.OMG.getMIRACLEs().gte(3e5), "cms_3"],
                [() => "Economists confirm: Celestial Market Saturation is not supposed to be survivable. And yet.", gameLayers.OMG.getMIRACLEs().gte(5e5) && Math.random() < 0.4, "cms_4"],
                [() => "Eishel audited the Celestial Market Saturation formula. Her report: 'HEI! This is fine actually. Keep going.'", gameLayers.OMG.getMIRACLEs().gte(2e5) && Math.random() < 0.35, "cms_5"],
                [() => "Saturation divider increasing. Fan Hearts unaffected. Songs unaffected. MIRACLEs unaffected in spirit.", gameLayers.OMG.getMIRACLEs().gte(4e5) && Math.random() < 0.3, "cms_6"],
                [() => "The K-pop universe said 'no more.' The fandom said 'watch us.' Both were correct.", gameLayers.OMG.getMIRACLEs().gte(2e5) && Math.random() < 0.35, "cms_7"],

                // Fandom Upgrades (generic fandom tab flavor)
                [() => "New Fandom upgrade purchased. The MIRACLEs feel it.", Math.random() < 0.3, "fandom_upg_1"],
                [() => "Fandom upgrade active. Fan Heart gain scaling improved. Euryle approves.", Math.random() < 0.3, "fandom_upg_2"],
                [() => "Breaking: A Fandom upgrade just made songs 9.17% more profitable. Yssabelle is thrilled.", Math.random() < 0.25, "fandom_upg_3"],
                [() => "Fandom upgrades are compounding. The MIRACLEs are evolving. This is expected and also alarming.", Math.random() < 0.25, "fandom_upg_4"],
                [() => "Eishel reviewed every Fandom upgrade description. She approved 8 of them. The other 1 she questioned loudly. It was valid.", Math.random() < 0.25, "fandom_upg_5"],
                [() => "Regina purchased a Fandom upgrade with exact precision. No wasted resources. She never wastes resources.", Math.random() < 0.2, "fandom_upg_6"],

                // Fan Hearts × Friends (crossover)
                [() => "Euryle's Fan Heart count is the highest in the group. She accepted this title with quiet dignity and a loud 'YooA #1!'", Math.random() < 0.35, "fanhearts_friends_1"],
                [() => "Alaine's Fan Hearts resonate at Heartlight frequency. Each one pulses gold. Scientists have given up explaining it.", Math.random() < 0.3, "fanhearts_friends_2"],
                [() => "Eishel generated Fan Hearts by saying 'HEI!' at the right frequency. This was not in the documentation.", Math.random() < 0.3, "fanhearts_friends_3"],
                [() => "Serille's Fan Hearts accumulated during tasks. She did not notice until they were already astronomical.", Math.random() < 0.25, "fanhearts_friends_4"],
                [() => "Regina's Fan Hearts are perfectly formatted. Each one is exactly the right size. She did this on purpose.", Math.random() < 0.25, "fanhearts_friends_5"],
                [() => "Kimberly's Fan Hearts are dark-colored. They still count. They count more, somehow.", Math.random() < 0.25, "fanhearts_friends_6"],
                [() => "Yssabelle discovered that Fan Hearts can be reinvested. She called this 'PALDO Hearts.' Fandom economists are intrigued.", Math.random() < 0.25, "fanhearts_friends_7"],
                [() => "Ahn Jean's Fan Hearts are shaped like Among Us crewmates. This is statistically improbable. Nobody is surprised.", Math.random() < 0.25, "fanhearts_friends_8"],

                // Rare Fandom
                [() => "RARE: Fan Hearts overflowed into a new dimension. Fandom tab now has a waiting room.", Math.random() < 0.04, "fandom_rare_1"],
                [() => "RARE: A single song generated enough Fan Hearts to power a small star. MIRACLEs were cited as the cause.", Math.random() < 0.03, "fandom_rare_2"],
                [() => "RARE: MIRACLE count briefly exceeded the number of particles in the universe. Celestial Market Saturation filed a complaint.", Math.random() < 0.025, "fandom_rare_3"],
                [() => "RARE: Euryle, Alaine, and Eishel listened to a song simultaneously. The Fan Heart counter reset to zero and then immediately went higher. Overflow.", Math.random() < 0.02, "fandom_rare_4"],
                [() => "RARE: Royalty income generated enough to purchase a moon. Yssabelle named it PALDO. It is canon.", Math.random() < 0.02, "fandom_rare_5"],
            ];
        },
        startTicker() {
